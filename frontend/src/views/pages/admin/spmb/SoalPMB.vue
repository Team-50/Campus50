	...
	<SPMBLayout>
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="orange" border="left" colored-border type="info">
					Berisi Soal PMB yang dikelompokan berdasarkan tahun akademik dan
					semester.
				</v-alert>
			</template>
		</ModuleHeader>
									...
									<v-form ref="frmdata" v-model="form_valid" lazy-validation>
										<v-card>
											...
											<v-card-text>
												...
												/>
												<v-divider class="mt-2" />
												<h3 class="headline mt-2 blue--text lighten-4">
													Jawaban Benar:
												</h3>
												<v-select
		...
		<template v-slot:filtersidebar>
			<Filter19
				...
				v-on:changeSemesterPendaftaran="changeSemesterPendaftaran"
				ref="filter19"
			/>
		</template>
	</SPMBLayout>
...
<script>
	...
	export default {
		...
		data: () => ({
			...
			rule_jawaban: [value => !!value || "Mohon isi jawaban dari soal ini"],
			rule_jawaban_benar: [
		...
		methods: {
			...
			async viewItem(item) {
				await this.$ajax
					...
					.then(({ data }) => {
						...
						if (this.formdata.gambar) {
							this.image_prev =
								this.$api.storageURL + "/" + this.formdata.gambar;
						}
					});
			},
			previewImage(e) {
			...
			save() {
				if (this.$refs.frmdata.validate()) {
					...
					if (this.editedIndex > -1) {
						...
						this.$ajax
							.post("/spmb/soalpmb/" + this.formdata.id, data, {
								headers: {
									Authorization: this.$store.getters["auth/Token"],
			...
			deleteItem(item) {
				this.$root.$confirm
					.open(
						"Delete",
						"Apakah Anda ingin menghapus data soal dengan ID " + item.id + " ?",
